...
<button type="submit" class="secondary" style="border-color:#b00;color:#b00">Полный сброс</button></form>
        ...
        # 4) Явка (отметки табеля) — удалить выбранные даты или всю явку.
        _att_row = ""
        _marks_sorted = sorted(emp.attendance_marks, key=lambda m: (m.mark_date, m.slot))
        if _marks_sorted:
            _checks = ""
            for _m in _marks_sorted:
                _checks += (f'<label style="display:block;margin:2px 0">'
                            f'<input type="checkbox" name="mark_ids" value="{_m.id}"> '
                            f'{_m.mark_date.strftime("%d.%m.%Y")} ({html.escape(_m.slot)}) — '
                            f'{html.escape(_m.code)}</label>')
            _att_row = f'''<div style="margin:6px 0;padding:8px;border:1px solid #f0d6d6;border-radius:8px;background:#fdf7f7">
<b>Явка ({len(_marks_sorted)} отметок)</b>
<form method="post" action="/employees/{emp.id}/event/delete" style="margin-top:6px">
<input type="hidden" name="event_kind" value="attendance">
<input type="hidden" name="target" value="attendance">
{_checks}
<button type="submit" name="mode" value="selected" class="secondary" style="margin-top:6px"
onclick="return confirm(&#39;Удалить отмеченные явки?&#39;)">Удалить выбранные</button>
<button type="submit" name="mode" value="all" class="secondary"
onclick="return confirm(&#39;Удалить ВСЮ явку сотрудника? Отметки табеля будут сняты (можно откатить по журналу).&#39;)">Удалить всю явку</button>
</form>
</div>'''

        if _del_rows or _full_reset_row or _att_row:
...
def employee_event_delete(
    ...
    event_kind: str = Form(...),
    target: str = Form(...),
    mode: str = Form(""),                       # для attendance: 'all' | 'selected'
    mark_ids: list[str] = Form(default=[]),      # для attendance mode='selected'
    db: Session = Depends(get_db),
):
    ...
    if event_kind == "attendance":
        # Удаление отметок явки (только админ — гейт выше). mode='all' → вся явка сотрудника;
        # mode='selected' → только отмеченные mark_ids. Физический DELETE, но полный снимок
        # каждой отметки пишется в аудит (target='attendance') — откат вставкой из snapshot.
        _marks = list(emp.attendance_marks)
        if mode == "selected":
            _sel = set(mark_ids)
            _marks = [m for m in _marks if m.id in _sel]
        if not _marks:
            return RedirectResponse(f"/employees/{employee_id}", status_code=303)
        _snap = [{
            "id": m.id,
            "mark_date": m.mark_date.isoformat(),
            "slot": m.slot,
            "code": m.code,
            "employee_name_snap": m.employee_name_snap,
            "created_by": m.created_by,
        } for m in _marks]
        for m in _marks:
            db.delete(m)
        db.add(EventDeletionLog(
            employee_id=employee_id, actor=actor, event_kind="attendance",
            target=("all" if mode == "all" else "selected"),
            snapshot=json.dumps({"marks": _snap}, ensure_ascii=False),
        ))
        db.commit()
        log.info("ADMIN %s удалил явку (%s) employee_id=%s: %d отметок",
                 actor, mode or "selected", employee_id, len(_snap))
        return RedirectResponse(f"/employees/{employee_id}", status_code=303)
