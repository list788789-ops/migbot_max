...
# Печать организации с подписью директора — общий документ (загружается на /common-docs),
# но НЕ входит в проверку комплектности пакета Госуслуг: там договор под ЭЦП, без картинки.
...
def _s3_client():
    ...
    Раньше клиент создавался на КАЖДЫЙ вызов — это главная причина медленной карточки (десятки
    S3-операций за рендер). Теперь клиент создаётся один раз и переиспользуется."""
...
def _s3_upload(scan_type: str, employee_id: str | None, data: bytes, content_type: str,
    ...
    """Загружает скан в бакет. metadata — пользовательские метаданные объекта (напр. пометка
    check=1 «требует проверки» для косячной платёжки). Бросает RuntimeError при проблемах."""
    client = _s3_client()
...
def _s3_list_for_employee(employee_id: str) -> dict:
    """Возвращает {scan_type: {"present": True, "check": bool}} для сканов работника.
    "check"=True — скан помечен «требует проверки» (косячная платёжка). Метка читается из
    метаданных того же head_object, что проверяет наличие — без дополнительных S3-запросов.
    Обратная совместимость: .get(type) правдиво/ложно как раньше (dict истинен, если есть)."""
    present = {}
    try:
        client = _s3_client()
    except RuntimeError:
