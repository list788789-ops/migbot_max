...
2026-07: сюда перенесён docx_to_pdf (был bot._docx_to_pdf) — печать согласия из веба тоже
должна отдавать PDF, а импортировать bot.py в webforms.py нельзя (потянет maxapi и второй
диспетчер). bot.py теперь импортирует его отсюда.

2026-07: добавлен generate_consents_batch_docx — один файл со ВСЕМИ бланками согласия
(по одному на страницу). Текст берётся из общего _build_consent_body, второй копии
формулировок 152-ФЗ в проекте нет и быть не должно.
...
def docx_to_pdf(docx_path: str, out_dir: str) -> str:
    ...
    функция — из бота вызывать через asyncio.to_thread, иначе на 2-4 сек замирает весь бот.
    В webforms вызывается напрямую (FastAPI-роут в def, а не async def, крутится в threadpool).

    2026-07: перенесена сюда из bot.py — webforms.py тоже нужен PDF (печать согласия из
    веба), а импортировать bot.py в webforms нельзя: потянется весь maxapi и второй
    экземпляр диспетчера. Один конвертер на оба входа, чтобы не разошлись поведением.
...
def generate_consent_docx(
    ...
    _build_consent_body(doc, employee, operator=operator, require_fields=require_fields)
...
def _build_consent_body(
    doc: Document,
    employee: Employee,
    operator: str = "tsm",
    require_fields: bool = True,
) -> None:
    """Наполняет ПЕРЕДАННЫЙ документ текстом одного согласия, ничего не сохраняя.

    Вынесено из generate_consent_docx (2026-07), чтобы пакетная печать
    (generate_consents_batch_docx) собирала бланки на всех работников из ОДНОГО источника
    текста. Второй копии формулировок 152-ФЗ в проекте быть не должно: разойдутся —
    и часть согласий уйдёт заказчику в устаревшей редакции."""
    if require_fields:
        _require_fields(employee, CONSENT_REQUIRED_FIELDS)
    else:
...
def generate_consents_batch_docx(
    employees: list[Employee],
    operator: str = "tsm",
    output_dir: str = "/tmp",
) -> str:
    """Один docx со ВСЕМИ бланками согласия — по одному на страницу (разрыв между ними).

    Зачем: по договору СМР (п.33.6) согласие нужно на каждого работника, вовлечённого в
    исполнение, бумагой с подписью и в 5 рабочих дней; за каждое непредоставленное —
    штраф 10 000 ₽ (п.28.1.27). При ~59 работниках печать по одному через карточку это
    59 кликов и 59 файлов, то есть гарантированные пропуски. Здесь — один файл на печать.

    Всегда require_fields=False: это пустографка под подпись, а неполнота карточки одного
    работника не должна ронять печать по всей бригаде (в строгом режиме первый же
    незаполненный паспорт поднял бы ValueError и не дал бы ни одного бланка).

    Порядок работников берётся как есть — сортировать должен вызывающий (по ФИО),
    чтобы бумажную пачку можно было разложить тем же порядком, что и список на экране."""
    if not employees:
        raise ValueError("Список работников пуст — печатать нечего.")
    ...
    for index, employee in enumerate(employees):
        if index:
            doc.add_page_break()
        _build_consent_body(doc, employee, operator=operator, require_fields=False)

    filename = f"consents_{operator}_{datetime.now(MSK).strftime('%Y%m%d_%H%M')}.docx"
    path = os.path.join(output_dir, filename)
    doc.save(path)
    return path
