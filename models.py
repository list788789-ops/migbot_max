...
2026-07 (третье изменение): раздел «Покупки» — Supplier, Nomenclature,
SupplierNomenclatureMap, SupplierParseProfile, GoodsReceipt, GoodsReceiptLine (в конце файла)
плюс роль UserRole.BUHGALTER. Таблицы новые, create_all создаст их сам; значение enum
и индексы — руками, инструкция в комментарии перед блоком.
"""
...
class UserRole(str, enum.Enum):
    ...
    PRORAB = "prorab"
    KADROVIK = "kadrovik"
    BUHGALTER = "buhgalter"   # 2026-07, раздел «Покупки»: поступления, номенклатура,
                              # поставщики. Не имеет доступа к работникам и медкомиссии.
    ADMIN = "admin"
...
# =============================================================================
# Раздел «Покупки» (2026-07) — поступление материалов от поставщиков.
#
# Появился из задачи: накладные приходят Excel-файлами, позиции заносятся руками.
# Подсистема разбирает файл (purchases/parser.py), сопоставляет строки с номенклатурой
# (purchases/matching.py) и создаёт документ, который человек подтверждает и выгружает
# в 1С через api_1c. Автопроведения нет: первичка — налоговые последствия, статус POSTED
# ставит только пользователь.
#
# Имя Invoice занято (счета клиники за медкомиссию, см. выше) — документ поступления
# называется GoodsReceipt. Домены не связаны, путать нельзя.
#
# МИГРАЦИЯ. Все таблицы ниже новые, Base.metadata.create_all() создаст их сам.
# Руками нужны только две вещи:
#   1) значение enum (create_all не меняет существующие типы):
#        ALTER TYPE userrole ADD VALUE IF NOT EXISTS 'buhgalter';
#      имя типа сверить: SELECT typname FROM pg_type WHERE typname ILIKE '%userrole%';
#      до PG 12 выполнять отдельным подключением с autocommit, не в транзакции;
#   2) индексы — см. блок в самом конце файла.
# =============================================================================

# --- Справочники ------------------------------------------------------------

class Supplier(Base):
    """Контрагент-поставщик. Отдельно от Employee и от clinic_id в Invoice —
    это внешняя организация, у которой покупаем материалы.

    inn — рабочий ключ во всей подсистеме: по нему находится профиль разбора файла
    и регистр соответствий номенклатуры. Уникален, но nullable: физлицо или разовая
    покупка без реквизитов встречается, отказывать в заведении из-за этого нельзя.
    """
    ...
    is_edo: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    # ↑ работает ли через ЭДО. Не косметика: по этому флагу считается доля объёма,
    # которую можно вывести из ручной загрузки вообще. Метрика для решения «строить
    # ли дальше», а не для отображения.
    note: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)


class Nomenclature(Base):
    """Ваш справочник номенклатуры. Термины по 1С, чтобы при выгрузке поля легли 1:1
    и не пришлось объяснять бухгалтерии, что такое «позиция».

    base_unit — единица хранения в каноническом виде из parser.canon_unit
    (шт/кг/т/м/м2/м3/л/компл/уп). Не «шт.» и не «ШТ» — сравнение единиц идёт строкой.

    article — ВАШ внутренний артикул. Артикулы поставщиков живут в
    SupplierNomenclatureMap, а не здесь: у одной позиции их столько же, сколько
    поставщиков, в одно поле они не влезают.

    is_active вместо удаления: на позицию уже могут ссылаться проведённые документы,
    физическое удаление порвёт историю.
    """
    ...
    base_unit: Mapped[str] = mapped_column(String(16), nullable=False, default="шт")
    vat_rate: Mapped[str | None] = mapped_column(String(8), nullable=True)  # "20%", "10%", "без НДС"
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)


class SupplierNomenclatureMap(Base):
    """Регистр соответствий: строка поставщика → ваша номенклатура.

    Это то, ради чего вся затея окупается. Первая накладная от поставщика требует
    ручного сопоставления, все следующие подтягиваются отсюда. Без этой таблицы
    загрузка остаётся ручным вводом с лишними шагами.

    supplier_key — нормализованный ключ из matching.supplier_key(): "a:<артикул>",
    если артикул есть, иначе "n:<нормализованное имя>". Нормализация ЖИВЁТ В КОДЕ
    (matching.norm_name), не в БД. Отсюда следствие, которое надо помнить: при
    изменении правил нормализации ключи протухают. Менять norm_name — значит
    пересчитывать эту таблицу миграцией, а не просто задеплоить.

    confirmed_by / confirmed_at — кто подтвердил связку. Записывать сюда результат
    нечёткого сопоставления БЕЗ подтверждения человеком нельзя: ошибка закрепится
    навсегда и будет тихо повторяться на каждой следующей накладной.
    """

    __tablename__ = "supplier_nomenclature_map"
    __table_args__ = (
        UniqueConstraint("supplier_id", "supplier_key", name="uq_supplier_key"),
    )

    id: Mapped[str] = mapped_column(String, primary_key=True, default=_uuid)
    supplier_id: Mapped[str] = mapped_column(ForeignKey("suppliers.id"), nullable=False)
    supplier_key: Mapped[str] = mapped_column(String, nullable=False)
    supplier_name_raw: Mapped[str] = mapped_column(String, nullable=False)   # как в файле, для показа
    supplier_article_raw: Mapped[str | None] = mapped_column(String, nullable=True)
    nomenclature_id: Mapped[str] = mapped_column(ForeignKey("nomenclature.id"), nullable=False)
...
class SupplierParseProfile(Base):
    """Раскладка Excel-файла конкретного поставщика: где шапка, какие колонки.

    Соответствует parser.SupplierProfile. columns хранится JSON-строкой в Text, а не
    в JSONB: остальная схема проекта на простых типах, а нагрузки на выборку по
    содержимому этого поля нет — читается целиком по supplier_id.

    Профиль перекрывает автопоиск шапки. Заполняется автоматически после первого
    удачного разбора (parser.profile_from_result) либо руками, когда эвристика
    не сработала и человек указал колонки в форме.
    """

    __tablename__ = "supplier_parse_profiles"
    ...
    skip_rows_after_header: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class ReceiptStatus(str, enum.Enum):
    """DRAFT — файл разобран, документ создан, но человек его не смотрел.
    REVIEW — есть расхождения (сумма не сошлась, несопоставленные строки, разошлись
    единицы) — требует внимания, отдельно от DRAFT, чтобы не терялось в списке.
    POSTED — бухгалтер подтвердил. С этого момента строки не редактируются.
    EXPORTED — выгружен в 1С через api_1c, есть onec_ref.
    CANCELLED — отменён (ошибочная загрузка, дубль, возврат).

    Автопроведения нет и не планируется: первичка — налоговые последствия, ответственность
    за цифры остаётся на человеке. POSTED ставит только пользователь.
    """
    DRAFT = "draft"
    REVIEW = "review"
    POSTED = "posted"
...
class GoodsReceipt(Base):
    """Поступление товаров и услуг — шапка документа. Название по 1С: при выгрузке
    ложится в одноимённый документ без переименования полей.

    file_hash UNIQUE — защита от повторной загрузки того же файла. Проверять ДО разбора:
    парсинг дороже, чем SELECT по индексу. Именно поэтому поле NOT NULL — документ
    в этой подсистеме всегда рождается из файла.

    scan_key — оригинал накладной в S3, по тому же механизму, что InternalOrder.scan_key
    и Certificate.scan_key (см. s3_storage._s3_upload_entity), scan_type = f"receipt_{id}".
    Хранить обязательно: спор с поставщиком решается сверкой с исходником, а не с тем,
    что распарсилось.

    amount_declared — итог, заявленный в файле; amount_lines — посчитанный по строкам.
    Хранятся ОБА намеренно. Расхождение — факт о документе, а не ошибка загрузки, и
    оно должно быть видно спустя год, а не только в момент загрузки.

    titul_id — привязка к объекту строительства. Nullable: не всякая поставка адресная,
    а требовать объект в момент загрузки — значит стопорить приёмку из-за аналитики.
    """
    ...
    number: Mapped[str | None] = mapped_column(String, nullable=True)   # номер накладной поставщика
    doc_date: Mapped[date | None] = mapped_column(Date, nullable=True)  # дата накладной
    titul_id: Mapped[str | None] = mapped_column(ForeignKey("tituly.id"), nullable=True)

    status: Mapped[ReceiptStatus] = mapped_column(
        Enum(ReceiptStatus), default=ReceiptStatus.DRAFT, nullable=False
    )

    amount_declared: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), nullable=True)
    amount_lines: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), nullable=True)
    vat_amount: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), nullable=True)

    file_hash: Mapped[str] = mapped_column(String(64), nullable=False, unique=True)
    file_name: Mapped[str | None] = mapped_column(String, nullable=True)
    scan_key: Mapped[str | None] = mapped_column(String, nullable=True)

    # Метрика качества на момент загрузки. Сохраняется, а не считается на лету:
    # справочник со временем меняется, и пересчёт задним числом даст другую цифру,
    # а нужна именно та, что была при приёмке.
    auto_matched: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    lines_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)

    warnings: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON-список из ParseResult
    onec_ref: Mapped[str | None] = mapped_column(String, nullable=True)  # ссылка/GUID в 1С после выгрузки

    created_by: Mapped[str | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    posted_by: Mapped[str | None] = mapped_column(ForeignKey("users.id"), nullable=True)
...
class GoodsReceiptLine(Base):
    ...
    Хранит И то, что пришло от поставщика (name_raw, article_raw, unit_raw), И результат
    сопоставления (nomenclature_id). Исходные значения не затираются никогда: по ним
    восстанавливается связка при пересопоставлении и решается спор о том, что именно
    было в накладной.

    nomenclature_id nullable — строка может остаться несопоставленной. Документ при этом
    существует в статусе REVIEW. Требовать сопоставления всех строк на входе значит
    блокировать приёмку из-за одной новой позиции.

    match_level — из matching.MatchLevel строкой ("LEARNED"/"ARTICLE"/"NAME_EXACT"/
    "FUZZY"/"NONE"). Строка, а не Enum в БД: набор уровней будет меняться при доработке
    правил, а ALTER TYPE на каждое изменение — лишняя боль.

    line_no — порядок в накладной, чтобы документ на экране совпадал с бумагой.
    row_in_file — номер строки Excel, чтобы при ошибке показать человеку, где смотреть.
    """

    __tablename__ = "goods_receipt_lines"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=_uuid)
    receipt_id: Mapped[str] = mapped_column(ForeignKey("goods_receipts.id"), nullable=False)
    line_no: Mapped[int] = mapped_column(Integer, nullable=False)
    row_in_file: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # как пришло от поставщика
    name_raw: Mapped[str] = mapped_column(String, nullable=False)
    article_raw: Mapped[str | None] = mapped_column(String, nullable=True)
    unit_raw: Mapped[str | None] = mapped_column(String(32), nullable=True)
    unit: Mapped[str | None] = mapped_column(String(16), nullable=True)  # канонизированная
    ...
    nomenclature_id: Mapped[str | None] = mapped_column(
        ForeignKey("nomenclature.id"), nullable=True
    )
    match_level: Mapped[str | None] = mapped_column(String(16), nullable=True)
    unit_mismatch: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
...
# --- Индексы ----------------------------------------------------------------
# create_all создаст таблицы и UNIQUE-констрейнты, но не эти индексы. Выполнить
# один раз руками; без них выборки поедут при росте, особенно первый.
#
...
#
# Последний нужен для аналитики «цена позиции во времени» — без него отчёт
# будет полным сканом строк по всем накладным.
